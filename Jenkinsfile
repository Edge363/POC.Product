...
node('dev') {         
    ...
    try {
        ...
        deleteSharedResources()
         for (application in applications) {
             sh """
                aws cloudformation delete-stack --stack-name ${application[0]}data
            """
            sh """
                aws cloudformation delete-stack --stack-name ${application[0]}service
            """
         }
    }
}
...
def deleteSharedResources(){
    sh """
        aws cloudformation delete-stack --stack-name sharednetworking --region us-east-1
    """
}